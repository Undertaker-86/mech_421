...
\renewcommand{\maketitle}{
  ...
  \begin{center}
    {\sffamily
      ...
      \vspace{0.6ex}
      {\normalsize Lab Performed:\ 31~October~2025}
    }
  \end{center}
  \vspace{1.2ex}
  \thispagestyle{empty}
...
\begin{infobox}[Calibrated Accuracy]
Applying \Cref{eq:thermistor_compensation} limits the residual error across the calibration baths to less than \SI{0.01}{\celsius}, comfortably satisfying the \SI{\pm0.15}{\celsius} goal.
\end{infobox}



\subsection{Exercise 2: Firmware and Desktop Acquisition}
The MSP430 firmware samples the thermistor channel at \SI{200}{\hertz}, averages eight consecutive readings, and formats the 10-bit result into the specified byte stream $[255,\mathrm{MS5B},\mathrm{LS5B}]$ per the MSP430FR57xx datasheet and user's guide~\cite{ti_msp430_ds,ti_msp430_user}. The essential routine is listed in \Cref{lst:msp430}, and Appendix~C captures the full project for traceability.
...
On the PC side, a C\# WinForms application reconstructs the ADC value, converts it to temperature via \Cref{eq:thermistor_beta,eq:thermistor_compensation}, streams the compensated temperature to disk, and renders live graphs. \Cref{lst:csharp} highlights the parsing and compensation block, while Appendix~C documents the capture/analysis pipeline, CSV logging, and tunable polynomial-calibration UI that were used to build the thermistor dataset.
...
\begin{figure}[H]
  \centering
  \includegraphics{figures/thermistor_ui.jpg}
  \caption{WinForms UI showing raw temperature, compensated value, and rolling waveform.}
...
\begin{infobox}[Thermal Response]
All measured heating and cooling steps settle in approximately \SI{10}{\second}, validating the first-order assumption used in the firmware and matching the boundary-layer thermal model.
\end{infobox}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/ntc/capture_20251031_183804_0_to_40_degC_heating.png}
    \caption{$0{\rightarrow}40~\si{\celsius}$ heat, $\tau=10.11~\si{\second}$}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\linewidth}
    \includegraphics[width=\linewidth]{figures/ntc/capture_20251031_183923_40_to_0_degC_cooling.png}
    \caption{$40{\rightarrow}0~\si{\celsius}$ cool, $\tau=10.16~\si{\second}$}
  \end{subfigure}
  \caption{Recorded temperature waveforms and first-order fits from \texttt{process\_ntc.py}; dashed lines mark the detected step, and the dotted verticals show the $1\tau$ crossing.}
  \label{fig:temp_waveform}
\end{figure}

Across all seven usable captures, the Python post-processing pipeline reports a weighted mean thermal time constant of $\bar{\tau} = \SI{10.09}{\second}$ with a sample standard deviation of \SI{0.08}{\second}, so $\tau_{\text{cal}} = \SI{10.09}{\second}$ is used when predicting step responses and aligning the plots in \Cref{fig:ntc_tau_overlay}.
...
Texas Instruments' SBOA237~\cite{ti_sboa237} and Analog Devices' 
AN-140~\cite{ad_an140}
both recommend the LT5400, where there are four resistors laser-trimmed on the same die. In that configuration the closed-loop gain becomes
\begin{equation}
...
m(\text{kg}) = -4.18 + 0.0193\, \text{ADC}_{10\text{b}},
\end{equation}
with $R^2 = 0.999$.
...
\begin{infobox}[Load-Cell Calibration]
The regression in \Cref{eq:weight_calibration} fits the calibration masses with $R^2 = 0.9992$ (approx.\ \SI{0.1}{\percent} linearity), so the desktop UI can report weight directly in kilograms without additional correction.
\end{infobox}
...
\begin{figure}[H]
  ...
  \caption{Inbuilt Load-cell calibration function.}
  \label{fig:calibration_plot_load_cell}
\end{figure}
...
Appendix~D documents the WinForms implementation, covering the UART parser, rolling-average and stability queues, tare handling, and the regression-based calibration workflow that backs the UI controls.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{figures/load_cell_calibrate.jpg}
  \caption{C\# UI screenshot - there are functions for tare, stability indicator, calibration and rolling weight plot.}
  \label{fig:weight_ui}
\end{figure}
The following two figures showcases the UI when loaded with 1 kg weight.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{figures/1kg_weight.jpg}
  \caption{C\# UI screenshot - there are functions for tare, stability indicator, calibration and rolling weight plot.}
  \label{fig:weight_ui}
\end{figure}
...
\bibitem{lab_manual}
Department of Mechanical Engineering, University of British Columbia, ``MECH 421/423 2025W Lab 3 Manual: Op Amp Circuits for DC Signal Processing,'' Oct.~2025.
\bibitem{ti_msp430_ds}
Texas Instruments, ``MSP430FR57xx Mixed-Signal Microcontrollers (Rev.~H),'' datasheet, 2020. Available: \url{https://www.ti.com/lit/ds/symlink/msp430fr5739.pdf}
\bibitem{ti_msp430_user}
Texas Instruments, ``MSP430FR57xx Family User's Guide (SLAU272O),'' 2021. Available: \url{https://www.ti.com/lit/pdf/slau272}
\bibitem{ti_sboa237}
Texas Instruments, ``SBOA237: Improving Gain Accuracy of Difference Amplifiers,'' Application Report, 2023.
\bibitem{ad_an140}
Analog Devices, ``AN-140: High Common-Mode Voltage Instrumentation Amplifier,'' Application Note, 2007.
\end{thebibliography}
...
\section*{Appendix C - Thermistor Acquisition Code}
\subsection*{MSP430FR5739 Firmware}
\noindent Listing~\ref{lst:thermistor_firmware_full} captures the complete firmware used for the thermistor channel. GPIO routing, the ADC10\_B setup, and Timer\_A cadence follow the FR57xx documentation, while the UART framing matches the $[255,\mathrm{MS5B},\mathrm{LS5B}]$ packet described in Exercise~2.

\lstinputlisting[language=C,basicstyle=\ttfamily\scriptsize,numbers=left,caption={Full MSP430FR5739 thermistor firmware (exported from \texttt{lab3/Ryan\_part1ex2/main.c}).},label=lst:thermistor_firmware_full]{appendix_sources/thermistor_main_export.c}

\subsection*{WinForms Capture Utility}
\noindent Listing~\ref{lst:thermistor_form_stream} shows the serial parser, Beta computation, polynomial correction, and live-plot update loop implemented in \texttt{Form1.cs}. Listing~\ref{lst:thermistor_form_capture} highlights the waveform capture manager, CSV export, and twin $\tau$ estimators (63\% crossing and log-domain regression) that feed the dataset summarized in \Cref{tab:ntc_summary}.

\lstinputlisting[language={[Sharp]C},basicstyle=\ttfamily\scriptsize,numbers=left,caption={Thermistor WinForms stream handler, compensation, and UI refresh logic (excerpt from \texttt{Form1.cs}).},label=lst:thermistor_form_stream]{appendix_sources/thermistor_form_stream.cs}

\lstinputlisting[language={[Sharp]C},basicstyle=\ttfamily\scriptsize,numbers=left,caption={Capture bookkeeping, CSV export, and dual $\tau$ estimators used by \texttt{process\_ntc.py}.},label=lst:thermistor_form_capture]{appendix_sources/thermistor_form_capture.cs}

\section*{Appendix D - Load-Cell Desktop Application}
\noindent The load-cell WinForms program reuses the same serial framing but layers on rolling averages, tare management, and a regression-based calibration workflow. Listing~\ref{lst:loadcell_stream} covers the byte parser, rolling queues that back the live plot, and the stability metric that drives the ``Stable'' badge. Listing~\ref{lst:loadcell_calibration} shows how calibration points are captured, filtered for stability, and converted into slope/intercept coefficients that update the UI in real time.

\lstinputlisting[language={[Sharp]C},basicstyle=\ttfamily\scriptsize,numbers=left,caption={Load-cell serial parser, rolling statistics, and chart updates (\texttt{Form1.cs} excerpt).},label=lst:loadcell_stream]{appendix_sources/loadcell_form_stream.cs}

\lstinputlisting[language={[Sharp]C},basicstyle=\ttfamily\scriptsize,numbers=left,caption={Calibration capture, regression, and tare handling routines for the load-cell UI.},label=lst:loadcell_calibration]{appendix_sources/loadcell_form_calibration.cs}
