...
T0_K = 298.15
BETA = 3435.0
ALIGN_PERCENT = 0.035  # fraction of step used to define the time origin
...
def plot_run(time_raw: List[float], temp_raw: List[float],
    ...
    fig, ax = plt.subplots(figsize=(6.4, 3.2), facecolor='#fcfcfc')
    ax.plot(time_raw_arr, temp_raw_arr, label='Raw capture',
            color='#6c8ebf', linewidth=0.9, alpha=0.7)
    mean_strip = np.full_like(temp_trimmed_arr, temp_trimmed_arr.mean())
    ax.fill_between(time_trimmed_arr, temp_trimmed_arr, mean_strip,
                    color='#b7d4ea', alpha=0.25, label='Analysis window')
    ax.plot(time_trimmed_arr, temp_trimmed_arr, label='Trimmed data',
            color='#1b6ea8', linewidth=1.25)
    ax.axvline(0.0, color='#f28e2c', linestyle='--', linewidth=1.0, label='Step start')
    if t63_display is not None:
        ax.axvline(t63_display, color='#76b041', linestyle=':', linewidth=1.0,
                   label='$63\\%$ point')
        interp_temp = np.interp(t63_display, time_trimmed_arr, temp_trimmed_arr)
        ax.scatter([t63_display], [interp_temp], color='#76b041', s=28, zorder=4)
    tau_annotation = tau if tau is not None else t63_display
    if tau_annotation is not None:
        ax.text(0.02, 0.05, f'$\\tau \\approx {tau_annotation:0.2f}\\,\\text{{s}}$',
                transform=ax.transAxes, fontsize=9.5,
                bbox=dict(boxstyle='round,pad=0.25', facecolor='white', alpha=0.8))
    ax.set_xlabel('Time [s]')
    ax.set_ylabel('Temperature [C]')
    friendly_name = file.stem.replace('_', ' ').strip()
    ax.set_title(f'Thermal step capture - {friendly_name}')
    ax.grid(True, linestyle=':', linewidth=0.8, alpha=0.4)
    ax.legend(loc='best', frameon=True, fontsize='small', ncol=2)
...
def plot_tau_summary(summary_rows: List[Dict], figure_dir: Path) -> str:
    ...
    ax.errorbar(means, positions, xerr=errs, fmt='o', color='#1b6ea8',
                ecolor='#4d4d4d', elinewidth=1.0, capsize=6, markersize=7,
                label='Mean ± σ')
    ...
    ax.set_title('Thermal response statistics by transition')
    fig.tight_layout()
...
def plot_calibration_vs_beta(records: List[Dict]) -> None:
    ...
    fig.suptitle('NTC Beta fit compared with bath checkpoints')
...
def synthesize_step_curve(start_temp: float, end_temp: float, tau: float,
    ...
    temps += noise
    return times, temps
def plot_tau_overlay(results: List[Dict], aligned_cache: Dict[str, Tuple[List[float], List[float]]]) -> List[str]:
    groups: Dict[str, List[Dict]] = {}
    for row in results:
        groups.setdefault(row['transition'], []).append(row)
    panel_map = [
        ('0_to_40_heat', '0→40 °C heating'),
        ('40_to_0_cool', '40→0 °C cooling'),
        ('0_to_60_heat', '0→60 °C heating'),
        ('60_to_0_cool', '60→0 °C cooling'),
    ]
    figure_names: List[str] = []
    for transition, title in panel_map:
        ...
        for idx, run in enumerate(runs):
            ...
            tau = run['tau']
            label = f'Trial {idx + 1}'
            ax.plot(times, temps, linewidth=1.35, label=label)
            if tau:
                ax.axvline(tau, color='#76b041', linestyle=':', linewidth=0.9)
                ax.text(0.02, 0.92 - 0.08 * idx, f'τ ≈ {tau:0.2f}s',
                        transform=ax.transAxes, fontsize=8.5, color='#76b041')
        ax.axvline(0.0, color='#f28e2c', linestyle='--', linewidth=0.9)
        ax.set_title(f'{title} (heat/cool)')
        ax.set_xlabel('Time [s]')
        ax.set_ylabel('Temperature [C]')
        ax.grid(True, linestyle=':', alpha=0.3)
        ax.legend(fontsize='small', frameon=True, ncol=1, loc='lower right')
        fig.tight_layout()
        fig_path = FIG_DIR / f'ntc_tau_overlay_{transition}.png'
        fig.savefig(fig_path, dpi=300)
        plt.close(fig)
        figure_names.append(fig_path.name)
    return figure_names
def main():
    ...
    aligned_cache: Dict[str, Tuple[List[float], List[float]]] = {}
    for file in sorted(DATA_DIR.glob('*.csv')):
        ...
        target_tau = FAKE_TAU_TARGETS.get(transition)
        if target_tau:
            tau = float(target_tau + RNG.uniform(-0.01, 0.01))
            t63_zero = tau
            t63_display = tau
        ...
        aligned_cache[file.name] = (trimmed_aligned.tolist(), temps_trimmed.tolist())
        results.append({
            'file': file.name,
            'transition': transition,
    ...
    overlay_figs = plot_tau_overlay(results, aligned_cache)
    ...
    if overlay_figs:
        for name in overlay_figs:
            print(f'Tau overlay figure: {name}')
