...
namespace ex8
{
    public partial class Form1 : Form
    {
        ...
        //    xy_right_hook
        //}
        private enum GestureState { idle, z_high_punch, x_right_hook, xy_right_hook }


        private GestureState current_gesture_state = GestureState.idle;
        private int gesture_timeout_counter = 0;
        private const int GESTURE_TIMEOUT_LIMIT = 100; // Timeout after x data points
        ...
        {
            // How come for this exercise I need to put the com ports in initialize form and not load?
            //comboBoxCOMPorts.Items.Clear();
            //comboBoxCOMPorts.Items.AddRange(System.IO.Ports.SerialPort.GetPortNames());
            //if (comboBoxCOMPorts.Items.Count <= 0)
            //{
        ...
        private void ProcessGesture(DataPoint point)
        {
            ...
            if (current_gesture_state != GestureState.idle)
            {
                ...
                {
                    if (current_gesture_state == GestureState.x_right_hook)
                    {
            ...
            switch (current_gesture_state)
            {
                ...
                case GestureState.z_high_punch:
                    {
                        if (point.ax > POS_X_THRESHOLD)
                        {
                ...
                case GestureState.xy_right_hook:
                    {
        ...
        private void LogHistory(string message)
        {
            ...
            {
                textBoxHistory.Invoke(new Action(() => LogHistory(message)));
            }
            else
            {
                textBoxHistory.AppendText(message + Environment.NewLine);
            }
        }
